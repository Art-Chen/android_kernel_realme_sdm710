...
#include "oppo_drv8834.h"
#include "../oppo_motor.h"

#define oppo_gpio_set_value(gpio,val) do { \
	if (gpio_is_valid(gpio))\
		gpio_set_value(gpio, val);\
} while(0)

#define oppo_gpio_free(gpio) do { \
	if (gpio_is_valid(gpio))\
		gpio_free(gpio);\
} while(0)

#define oppo_gpio_get_value(gpio) \
	((gpio_is_valid(gpio)) ? (gpio_get_value(gpio)) : ERROR_NO)

static struct oppo_mdrv_chip *g_chip = NULL;
static void drv8834_check_motor_type(struct oppo_mdrv_chip *chip);

static int drv8834_parse_dts(struct oppo_mdrv_chip * chip)
{
	...
	if (!gpio_is_valid(chip->m1_gpio)) {
		...
		if (rc)
			...
			oppo_gpio_set_value(chip->m1_gpio, 0);
	}
	...
	if (!gpio_is_valid(chip->vref_gpio)) {
		...
		if (rc)
			...
			oppo_gpio_set_value(chip->vref_gpio, 0);
	}
	...
	if (!gpio_is_valid(chip->sleep_gpio)) {
		...
		if (rc)
			...
			oppo_gpio_set_value(chip->sleep_gpio, 0);
	}
	...
	if (!gpio_is_valid(chip->sleep1_gpio)) {
		...
		if (rc)
			...
			oppo_gpio_set_value(chip->sleep1_gpio, 0);
	}
	...
	if (!gpio_is_valid(chip->dir_gpio)) {
		...
		if (rc)
			...
			oppo_gpio_set_value(chip->dir_gpio, 0);
	}
	...
	if (!gpio_is_valid(chip->m0_gpio)) {
		...
		if (rc)
			...
			oppo_gpio_set_value(chip->m0_gpio, 0);
	}
	...
	MOTOR_LOG("%d %d %d %d %d %d %d %d %d [%d %d %d %d]\n",chip->boost_gpio,chip->vref_gpio,chip->sleep_gpio,chip->sleep1_gpio,
			chip->dir_gpio,chip->m0_gpio,chip->m1_gpio,chip->step_gpio,chip->dir_switch_gpio,
			chip->ratio_a,chip->ratio_b,chip->ratio_c,chip->ratio_d);

	return 0;
}

static int drv8834_init_dir_switch(struct oppo_mdrv_chip * chip)
{
	int ret = 0;
	...
	chip->dir_switch = oppo_gpio_get_value(chip->dir_switch_gpio);
...
static int drv8834_init_default(struct oppo_mdrv_chip * chip)
{
	int ret = 0;
...
static int drv8834_init_m1_gpio(struct oppo_mdrv_chip * chip)
{
	int ret = 0;
...
static int drv8834_init_active_config(struct oppo_mdrv_chip * chip)
{
	int ret = 0;
...
static int drv8834_init_sleep_config(struct oppo_mdrv_chip * chip)
{
	int ret = 0;
...
static int drv8834_change_m0_config(struct oppo_mdrv_chip * chip ,int config)
{
	int ret = 0;
...
static int drv8834_change_vref_config(struct oppo_mdrv_chip * chip ,int config)
{
	int ret = 0;
...
static int drv8834_hardware_init(struct oppo_mdrv_chip * chip)
{
	int ret = 0;
...
static int drv8834_set_power(int mode)
{
	...
	if (mode == MOTOR_POWER_ON) {
		oppo_gpio_set_value(g_chip->boost_gpio, 1);//boost should be always on
		oppo_gpio_set_value(g_chip->sleep_gpio, 1);
		oppo_gpio_set_value(g_chip->sleep1_gpio, 1);
	} else {
		oppo_gpio_set_value(g_chip->sleep_gpio, 0);
		oppo_gpio_set_value(g_chip->sleep1_gpio, 0);
		oppo_gpio_set_value(g_chip->step_gpio, 0);
		oppo_gpio_set_value(g_chip->dir_gpio, 0);
	}
	return 0;
}
...
static int drv8834_set_direction(int dir)
{
	...
	if ((g_chip->dir_switch != ERROR_NO) && (g_chip->dir_switch)) {//for findx
		if (dir == MOTOR_UPWARD) {
			oppo_gpio_set_value(g_chip->dir_gpio, 1);
		} else {
			oppo_gpio_set_value(g_chip->dir_gpio, 0);
		}
	} else {
		if (oppo_get_dir_sign() == POSITIVE) {
			if (dir == MOTOR_UPWARD) {
				oppo_gpio_set_value(g_chip->dir_gpio, 1);
			} else {
				oppo_gpio_set_value(g_chip->dir_gpio, 0);
...
static int drv8834_set_working_mode (int mode)
{
	...
	case MOTOR_MODE_FULL:
		...
		oppo_gpio_set_value(g_chip->vref_gpio, 0);
		oppo_gpio_set_value(g_chip->m0_gpio, 0);
		oppo_gpio_set_value(g_chip->m1_gpio, 0);
		break;
	case MOTOR_MODE_1_16:
		...
		oppo_gpio_set_value(g_chip->m0_gpio, 1);
		oppo_gpio_set_value(g_chip->m1_gpio, 1);
		break;
	...
	MOTOR_ERR("config change %d %d %d %d %d %d\n",
		oppo_gpio_get_value(g_chip->sleep_gpio),
		oppo_gpio_get_value(g_chip->step_gpio),
		oppo_gpio_get_value(g_chip->m0_gpio),
		oppo_gpio_get_value(g_chip->m1_gpio),
		oppo_gpio_get_value(g_chip->vref_gpio),
		oppo_gpio_get_value(g_chip->dir_gpio));
	return 0;
}
...
static int mtk_pwm_config(int duty_ns, int period_ns, uint32_t wave_num)
{
	...
	L_duration = H_duration = 13* period_ns / (2*1000) - 1; /* (26*period_ns)/(2*2*1000) - 1 */
...
static void drv8834_check_motor_type(struct oppo_mdrv_chip *chip)
{
    chip->motor_type = oppo_get_motor_type();
...
static int  drv8834_get_all_config(int* config ,int count)
{
	...
	config[0] = oppo_gpio_get_value(g_chip->sleep_gpio);
	config[1] = oppo_gpio_get_value(g_chip->step_gpio);
	config[2] = oppo_gpio_get_value(g_chip->m0_gpio);
	config[3] = oppo_gpio_get_value(g_chip->m1_gpio);
	config[4] = oppo_gpio_get_value(g_chip->vref_gpio);
	config[5] = oppo_gpio_get_value(g_chip->dir_gpio);
...
struct oppo_motor_operations  drv8834_ops = {
...
{
	struct oppo_mdrv_chip *chip = NULL;
	int ret = 0;
	int id = -1;

	MOTOR_LOG("call\n");

	chip = devm_kzalloc(&pdev->dev,sizeof(struct oppo_mdrv_chip), GFP_KERNEL);
	if (!chip) {
	...
	oppo_register_motor("drv8834", &drv8834_ops);
	id = oppo_get_driver_ic_id();
...
{
	if (g_chip) {
		oppo_gpio_free(g_chip->boost_gpio);
		oppo_gpio_free(g_chip->vref_gpio);
		oppo_gpio_free(g_chip->dir_gpio);
		oppo_gpio_free(g_chip->m0_gpio);
		oppo_gpio_free(g_chip->m1_gpio);
		oppo_gpio_free(g_chip->sleep_gpio);
		oppo_gpio_free(g_chip->sleep1_gpio);
		kfree(g_chip);
		g_chip = NULL;
	}
	return 0;
