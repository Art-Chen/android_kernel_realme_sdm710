...
static struct oppo_lt_chip *g_chip = NULL;
static void lt3572_check_motor_type(struct oppo_lt_chip *chip);
static int lt3572_set_working_mode (int mode);
static int lt3572_set_direction(int dir);

static int lt3572_parse_dts(struct oppo_lt_chip * chip)
{
	...
	if (!gpio_is_valid(chip->shdn_gpio)) {
		...
		if (rc)
			...
			oppo_gpio_set_value(chip->shdn_gpio, 0);
	}
	...
	if (!gpio_is_valid(chip->shdna_gpio)) {
		...
		if (rc)
			...
			oppo_gpio_set_value(chip->shdna_gpio, 0);
	}
	...
	if (!gpio_is_valid(chip->shdnb_gpio)) {
		...
		if (rc)
			...
			oppo_gpio_set_value(chip->shdnb_gpio, 0);
	}
	...
	MOTOR_LOG("%d %d %d %d [%d]\n",chip->step_gpio,chip->shdn_gpio,chip->shdna_gpio,chip->shdnb_gpio,chip->ratio_a);

	return 0;
}

static int lt3572_init_pwm_config(struct oppo_lt_chip * chip)
{
	int ret = 0;
...
static int lt3572_init_shdn_gpio(struct oppo_lt_chip * chip)
{
	int ret = 0;
...
static int lt3572_init_shdna_gpio(struct oppo_lt_chip * chip)
{
	int ret = 0;
...
static int lt3572_init_shdnb_gpio(struct oppo_lt_chip * chip)
{
	int ret = 0;
...
static int lt3572_hardware_init(struct oppo_lt_chip * chip)
{
	int ret = 0;
...
static int lt3572_set_power(int mode)
{
	...
	if (mode == MOTOR_POWER_ON) {
		...
		oppo_gpio_set_value(g_chip->shdn_gpio, 1);
		oppo_gpio_set_value(g_chip->shdna_gpio, 1);
		oppo_gpio_set_value(g_chip->shdnb_gpio, 1);
	} else {
		oppo_gpio_set_value(g_chip->shdn_gpio, 0);
		oppo_gpio_set_value(g_chip->shdna_gpio, 0);
		oppo_gpio_set_value(g_chip->shdnb_gpio, 0);
	}
	return 0;
}

static void lt3572_set_duty_ratio(struct oppo_lt_chip * chip, enum DUTY_RATIO ratio)
{
...
static int  lt3572_get_all_config(int* config ,int count)
{
	...
	config[0] = oppo_gpio_get_value(g_chip->step_gpio);
	config[1] = oppo_gpio_get_value(g_chip->shdn_gpio);
	config[2] = oppo_gpio_get_value(g_chip->shdna_gpio);
	config[3] = oppo_gpio_get_value(g_chip->shdnb_gpio);
...
struct oppo_motor_operations  lt3572_ops = {
...
{
	struct oppo_lt_chip *chip = NULL;
	int ret = 0;
	int id = -1;

	MOTOR_LOG("call\n");

	chip = devm_kzalloc(&pdev->dev,sizeof(struct oppo_lt_chip), GFP_KERNEL);
	if (!chip) {
	...
	oppo_register_motor("lt3572",&lt3572_ops);
...
{
	if (g_chip) {
		oppo_gpio_free(g_chip->step_gpio);
		oppo_gpio_free(g_chip->shdn_gpio);
		oppo_gpio_free(g_chip->shdna_gpio);
		oppo_gpio_free(g_chip->shdnb_gpio);
		kfree(g_chip);
		g_chip = NULL;
	}
	return 0;
