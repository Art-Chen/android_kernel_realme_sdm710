...
static struct oppo_sts_chip *g_chip = NULL;
static void stspin220_check_motor_type(struct oppo_sts_chip *chip);
static int stspin220_set_working_mode (int mode);
static int stspin220_set_direction(int dir);

static int stspin220_parse_dts(struct oppo_sts_chip * chip)
{
	...
	if (!gpio_is_valid(chip->step_gpio)) {
		...
		//else
		//	oppo_gpio_set_value(chip->step_gpio, 0);
	}
	...
	if (!gpio_is_valid(chip->enable_gpio)) {
		...
		if (rc)
			...
			oppo_gpio_set_value(chip->enable_gpio, 0);
	}
	...
	if (!gpio_is_valid(chip->standby_gpio)) {
		...
		if (rc)
			...
			oppo_gpio_set_value(chip->standby_gpio, 0);
	}
	...
	if (oppo_is_support_mode()) {
		...
		if (!gpio_is_valid(chip->mode1_gpio)) {
			...
			if (rc)
				...
				oppo_gpio_set_value(chip->mode1_gpio, 0);
		}
	}
	...
	MOTOR_LOG("%d %d %d %d %d [%d %d %d %d]\n",chip->boost_gpio,chip->standby_gpio,
...
static int stspin220_init_boost(struct oppo_sts_chip * chip)
{
	int ret = 0;
...
static int stspin220_init_standby_gpio(struct oppo_sts_chip * chip)
{
	int ret = 0;
...
static int stspin220_init_enable_gpio(struct oppo_sts_chip * chip)
{
	int ret = 0;
...
static int stspin220_init_pwm_config(struct oppo_sts_chip * chip , int is_pwm_mode)
{
	int ret = 0;
...
static int stspin220_init_dir_gpio(struct oppo_sts_chip * chip)
{
	int ret = 0;
...
static int stspin220_init_mode1_gpio(struct oppo_sts_chip * chip)
{
	int ret = 0;

	if (oppo_is_support_mode() == false) {
		return 0;
	}
...
static int stspin220_init_active_config(struct oppo_sts_chip * chip)
{
	int ret = 0;
...
static int stspin220_init_sleep_config(struct oppo_sts_chip * chip)
{
	int ret = 0;
...
static int stspin220_hardware_init(struct oppo_sts_chip * chip)
{
	int ret = 0;
	...
	oppo_gpio_set_value(chip->boost_gpio, 0);
...
static int stspin220_set_power(int mode)
{
	...
	if (mode == MOTOR_POWER_ON) {
		...
		oppo_gpio_set_value(g_chip->enable_gpio, 1);
	} else {
		oppo_gpio_set_value(g_chip->enable_gpio, 0);
		oppo_gpio_set_value(g_chip->standby_gpio, 0);
	}
	return 0;
}
...
static int stspin220_set_working_mode (int mode)
{
	...
	usleep_range(14,15);
	oppo_gpio_set_value(g_chip->standby_gpio, 1);

	MOTOR_ERR("config change %d %d\n",
		oppo_gpio_get_value(g_chip->standby_gpio),
...
static int  stspin220_get_all_config(int* config ,int count)
{
	...
	if (oppo_is_support_mode()) {
		config[0] = oppo_gpio_get_value(g_chip->mode1_gpio);
	} else {
		config[0] = 0;
	}
	config[1] = oppo_gpio_get_value(g_chip->step_gpio);
	config[2] = oppo_gpio_get_value(g_chip->standby_gpio);
	config[3] = oppo_gpio_get_value(g_chip->enable_gpio);
	config[4] = oppo_gpio_get_value(g_chip->dir_gpio);
...
struct oppo_motor_operations  stspin220_ops = {
...
{
	struct oppo_sts_chip *chip = NULL;
	int ret = 0;
	int id = -1;

	MOTOR_LOG("call\n");

	chip = devm_kzalloc(&pdev->dev,sizeof(struct oppo_sts_chip), GFP_KERNEL);
	if (!chip) {
	...
	oppo_register_motor("stspin220",&stspin220_ops);
...
fail:
	MOTOR_ERR("fail \n");
	oppo_unregister_motor("stspin220");
...
static int stspin220_platform_remove(struct platform_device *pdev)
{
	if (g_chip) {
		...
		oppo_gpio_free(g_chip->enable_gpio);
		if (oppo_is_support_mode()) {
			oppo_gpio_free(g_chip->mode1_gpio);
		}
		oppo_gpio_free(g_chip->standby_gpio);
		kfree(g_chip);
		g_chip = NULL;
	}
	return 0;
