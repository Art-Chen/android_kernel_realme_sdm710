...
__attribute__((weak)) void oppo_m1120_reconfig(struct oppo_dhall_chip *chip) {return;}

static int m1120_i2c_read_block(struct oppo_dhall_chip *chip, u8 addr, u8 *data, u8 len)
{
	u8 reg_addr = addr;
	int err = 0;
...
static int m1120_i2c_write_block(struct oppo_dhall_chip *chip, u8 addr, u8 *data, u8 len)
{
	int err = 0;
	int idx = 0;
	int num = 0;
...
static void m1120_short_to_2byte(struct oppo_dhall_chip *chip, short x, u8 *hbyte, u8 *lbyte)
{
...
static short m1120_2byte_to_short(struct oppo_dhall_chip *chip, u8 hbyte, u8 lbyte)
{
	short x = 0;
...
static int m1120_get_id(struct oppo_dhall_chip *chip)
{
	u8 data = 0;
...
static int m1120_set_power_gpio_down(struct oppo_dhall_chip * chip)
{
	int ret = 0;
...
static int m1120_set_power_gpio_up(struct oppo_dhall_chip * chip)
{
	int ret = 0;
...
static int m1120_set_power(struct oppo_dhall_chip *chip, bool on)
{
	int ret = 0;
...
static int m1120_set_frequency(struct oppo_dhall_chip *chip,int frequency)
{
	int err = 0;
	u8 rdata = 0;
...
static int m1120_clear_interrupt(struct oppo_dhall_chip *chip)
{
    int ret = 0;
...
static bool m1120_update_threshold(unsigned int id, int position, short lowthd, short highthd)
{
...
static int m1120_set_operation_mode(struct oppo_dhall_chip *chip, int mode)
{
...
int m1120_setup_eint(struct oppo_dhall_chip *chip)
{
	int ret = 0;
...
static int m1120_enable_hidden(struct oppo_dhall_chip *chip, bool enable, uint32_t ratio)
{
	int err = 0;
	...
	err = m1120_i2c_write_block(chip, M1120_REG_HIDDEN, &data, 1); /*enable hidden register*/
	if (err < 0) {
...
static int m1120_reset_device(struct oppo_dhall_chip *chip)
{
	int err = 0;
	u8 data =0;
...
static void m1120_parse_dts(struct oppo_dhall_chip * chip)
{
...
struct oppo_dhall_operations  m1120_ops = {
...
static int m1120_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	oppo_register_dhall("m1120",&m1120_ops);
